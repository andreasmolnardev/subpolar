...
}, ['query', 'results', 'context', 'provider'])
...
export const SUBPOLAR_TOOL_SEEDS: ToolSeed[] = [
  ...
  { tool_id: 'web.search', namespace: 'web', description: 'Search the public web for research sources with Exa MCP', adapter: 'internal', target: 'web', operation: 'search', input_schema: objectSchema({ query: { type: 'string' }, limit: { type: 'number' }, type: { type: 'string' }, livecrawl: { type: 'string' }, contextMaxCharacters: { type: 'number' } }, ['query']), output_schema: webSearchOutput, risk: 'external', requires_approval: false, enabled: true, metadata: { examples: [{ query: 'Odysseus deep research agent implementation', limit: 5, type: 'auto', livecrawl: 'fallback' }] } },
