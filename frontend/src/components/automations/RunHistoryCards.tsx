...
import { getRunTone } from '@/components/automations/automation-utils'
import { RunDetailPanel } from '@/components/automations/RunDetailPanel'
import { useRepoAutomationRun } from '@/hooks/useAutomations'
...
export function RunHistoryCards({
  ...
  const { data: runDetail, isLoading } = useRepoAutomationRun(
    expandedRunRepoId ?? undefined,
    expandedRunJobId,
    expandedRunId
  )

  function getRunStatusIcon(status: AutomationRun['status']) {
