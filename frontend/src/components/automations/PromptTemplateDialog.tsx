      ...
      <DialogContent className=" flex h-[calc(100dvh-1rem)] max-h-[calc(100dvh-1rem)] w-[calc(100vw-1rem)] max-w-2xl flex-col overflow-hidden sm:h-auto sm:max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <Input id="template-suggested-description" value={form.suggestedDescription} onChange={handleChange('suggestedDescription')} placeholder="Pre-fills the automation job description field" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="template-prompt">Prompt</Label>
            <Textarea id="template-prompt" value={form.prompt} onChange={handleChange('prompt')} className="min-h-[200px]" placeholder="The full prompt sent to the agent when the automation runs." />
          </div>
        </div>
