...
type ProjectSkillsDialogBaseProps = {
  open: boolean
...
export function ProjectSkillsDialog({
  ...
  const { isLoading, data, error } = useQuery({
    queryKey: directory ? ['settings', 'skills', 'directory', directory] : ['settings', 'skills', projectId],
    queryFn: () => settingsApi.listManagedSkills(projectId, directory),
    enabled: open && (!!projectId || !!directory),
    staleTime: 30000,
  })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent mobileFullscreen className="sm:max-w-2xl sm:max-h-[85vh] gap-0 flex flex-col p-0 md:p-6 pb-safe">
        <DialogHeader className="p-4 sm:p-6 border-b shrink-0">
          ...
          <DialogDescription>
            {canLoad ? 'Load a skill into the current session' : 'Skills available for this project'}
          </DialogDescription>
        </DialogHeader>
